...
const { OnlineAgents, ChatMessages, ChatConversations , ChatQueue } = require("../models/index.js");
...
const setupSocket = (server) => {
  ...
  console.log("✅ Socket.IO server initialized on port", process.env.SOCKETPORT);
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("register_agent", async ({ agentId , agentName }) => {
      try {
        const agent = await OnlineAgents.findOne({ where: { agent_id: agentId } });
        // console.log(agent)
        if (agent) {
          ...
          console.log(`Agent ${agentId} registered with socketId: ${socket.id}`);
          socket.emit("agent_registered", { msg: "تم تسجيل الوكيل بنجاح" });
        } else {
          const agent = await OnlineAgents.create({agent_name : agentName , socket_id: socket.id , agent_id : agentId , last_ping: new Date(), is_available: true });
          console.log(`Agent ${agent.id} registered with socketId: ${socket.id}`);
    ...
    socket.on("join_chat", async ({ chat_id, userId, userType }) => {
      try {
        if (!userId || !userType || !chat_id) {
          socket.emit("error", { msg: "يجب إرسال userId و userType و chat_id" });
          return;
        }
    ...
    socket.on("send_chat_message", async ({ chat_id, message, file_url, message_type = "text" }, callback) => {
      try {
        ...
        const conversation = await ChatConversations.findOne({
          where: {
            ...
            [socket.userType === "customer" ? "customer_id" : "crm_agent_id"]: socket.userId,
          },
        });

        if (!conversation) {
          socket.emit("error", { msg: "المحادثة غير موجودة أو مغلقة" });
          return callback && callback({ error: "المحادثة غير موجودة أو مغلقة" });
        }
  ...
  setInterval(async () => {
    try {
      ...
      await ChatQueue.destroy({ where: { conversation_id: conversation.id } });
      ...
      console.log(`Conversation ${conversation.id} assigned to agent ${agentId}`);
    } catch (error) {
